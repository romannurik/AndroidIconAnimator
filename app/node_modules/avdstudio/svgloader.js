import {Artwork, LayerGroup, Layer, IdProperty} from './model';
...
export const SvgLoader = {
  loadArtworkFromSvgString(svgString) {
    ...
    let nodeToLayerData_ = (node, context) => {
      ...
      simpleAttr_('stroke-opacity', 'strokeOpacity');
      simpleAttr_('fill', 'fillColor');
      simpleAttr_('fill-opacity', 'fillOpacity');
      ...
      if (path) {
        ...
        if (context.transforms && context.transforms.length) {
          ...
          if (parsed) {
            parsed.commands.forEach(({command, args}) => {
              if (command == '__arc__') {
                // TODO: transform arcs
                return;
              }
              ...
              for (let i = 0; i < args.length; i += 2) {
                let transformed = transformPoint_({x:args[i], y:args[i + 1]}, context.transforms);
                args[i] = transformed.x;
                args[i + 1] = transformed.y;
              }
            });

            path = SvgPathParser.parsedPathToString(parsed);
          }
        }
