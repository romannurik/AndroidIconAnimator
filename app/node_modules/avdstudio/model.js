...
export class PathDataProperty extends Property {
  interpolateValue(start, end, f) {
    ...
    // console.log('end', end);
    start = (typeof start === 'string') ? SvgPathParser.parse(start) : start;
    end = (typeof end === 'string') ? SvgPathParser.parse(end) : end;;
    return SvgPathParser.interpolateParsedPaths(start, end, f);
  }
  ...
  getEditableValue(obj, propertyName) {
    return obj[propertyName]
        ? (typeof obj[propertyName] == 'string' ? obj[propertyName] : obj[propertyName].string)
        : '';
  }
  ...
  setter_(obj, propertyName, value) {
    ...
    let parsedPath;
    if (typeof value === 'string') {
      parsedPath = SvgPathParser.parse(value);

    } else if (!value || 'string' in value) {
      parsedPath = value; // either a null/undefined value or a full parsed path

    } else if (Array.isArray(value)) {
      // just the commands
      parsedPath = obj[backingPropertyName] || {};
      parsedPath.commands = value;
      // TODO: compute path length
    }

    obj[backingPropertyName] = parsedPath;
  }
