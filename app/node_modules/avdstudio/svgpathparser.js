...
export const SvgPathParser = {
  parse(pathString) {
    let parsedPath = {
      string: pathString
    };

    parsedPath.commands = SvgPathParser.parseCommands_(pathString);
    Object.assign(parsedPath, computePathLengthAndBounds_(parsedPath.commands));
    return parsedPath;
  },

  parseCommands_(pathString) {
    let commands = [];
  ...
  execute(ctx, parsedPath) {
    ctx.beginPath();
    parsedPath.commands.forEach(({command, args}) => {
      if (command == '__arc__') {
        SvgPathParser.executeArc_(ctx, args);
      } else {
        ctx[command](...args);
      }
    });
  ...
  interpolateParsedPaths(start, end, f) {
    ...
    for (i = 0; i < start.commands.length; i++) {
      ...
      interpolated.commands.push({
        command: si.command,
        args: interpolatedArgs
      });
    }

    interpolated.string = SvgPathParser.parsedPathToString(interpolated);
    Object.assign(interpolated, computePathLengthAndBounds_(interpolated.commands));
    return interpolated;
  },

  parseAndExecute(ctx, pathString) {
    SvgPathParser.execute(ctx, SvgPathParser.parse(pathString));
  }
};
...
let computePathLengthAndBounds_ = commands => {
  let length = 0;
...
let arcToBeziers_ = (xf, yf, rx, ry, rotate, largeArcFlag, sweepFlag, xt, yt) => {
...
*/
let unitCircleArcToBeziers_ = (angleStart, angleExtent) => {
...
window.SvgPathParser = SvgPathParser;